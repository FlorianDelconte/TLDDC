...
void
DefectSegmentationUnroll_copy::init(){
    allocate();
...
void
DefectSegmentationUnroll_copy::allocateExtra(){
  coefficients.resize(pointCloud.size());
}

void
DefectSegmentationUnroll_copy::computeEquations(){
  ...
  for(int i = 0; i < nbCores - 1; i++){
      ts.push_back(std::thread(&DefectSegmentationUnroll_copy::computeEquationsMultiThread, this, i, nbCores, kdtree, minH, maxH));
  }
...
DefectSegmentationUnroll_copy::computeEq(unsigned int idPoint,double searchRadius, double patchAngle,const pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree){
...
void
DefectSegmentationUnroll_copy::computeEquationsMultiThread(int threadId, int nbThread,const pcl::KdTreeFLANN<pcl::PointXYZ> &kdtree, double minH, double maxH){
...
void
DefectSegmentationUnroll_copy::computeDistances(){
...
void
DefectSegmentationUnroll_copy::getDefect(std::string outputFileName){
...
void 
DefectSegmentationUnroll_copy::createVisuImage(std::string s,cv::Mat c){
